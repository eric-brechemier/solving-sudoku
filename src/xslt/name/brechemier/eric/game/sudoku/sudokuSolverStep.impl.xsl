...
>
  ...
  <xsl:function name="sudoku:value-or-allowed-value-on-different-square-in-region" as="node()*">
    ...
    <xsl:for-each select="$contextNode">
      <xsl:for-each select="key('region-square',$region)[not(@row=$row and @col=$col)]">
        <xsl:copy-of select="@value | sudoku:allowed/sudoku:symbol"/>
      </xsl:for-each>
    </xsl:for-each>
  </xsl:function>
  ...
  <xsl:template match="sudoku:sudoku[ sudoku:square[not(@value)] ]" priority="2">
  ...
  <xsl:template match="sudoku:square" priority="1">
    <xsl:param name="step" />
    <square>
  ...
  <xsl:template mode="scanning" match="sudoku:square[   $allSymbolsCount   -   count(  distinct-values( key('row-value',@row)|key('col-value',@col)|key('region-value',@region) )  )   =1   ]" priority="2">
    ...
    <xsl:attribute name="value">
      <xsl:value-of select="$allSymbols[not(  .=( key('row-value',$thisRow)|key('col-value',$thisCol)|key('region-value',$thisRegion) )  )]"/>
    </xsl:attribute>
    <xsl:attribute name="method">1-scanning</xsl:attribute>
    <xsl:attribute name="step"><xsl:value-of select="$step"/></xsl:attribute>
  </xsl:template>
  <xsl:template mode="scanning" match="sudoku:square" priority="1">
    <xsl:param name="step" />
  ...
  <xsl:template mode="marking-up" match="sudoku:square" priority="1">
    ...
    <allowed>
      <xsl:apply-templates mode="marking-up" select="$allSymbols[not(  .=( key('row-value',$thisRow)|key('col-value',$thisCol)|key('region-value',$thisRegion) )  )]"/>
    </allowed>
  </xsl:template>
  ...
  <xsl:template mode="pattern-matching" match="sudoku:square[count( sudoku:allowed/sudoku:symbol[not(.=../../sudoku:forbidden/sudoku:symbol)] )=1]" priority="2">
    <xsl:param name="step" />
    <xsl:attribute name="value">
      <xsl:value-of select="sudoku:allowed/sudoku:symbol[not(.=../../sudoku:forbidden/sudoku:symbol)]"/>
    </xsl:attribute>
    <xsl:attribute name="method">3-pattern-matching</xsl:attribute>
    <xsl:attribute name="step"><xsl:value-of select="$step"/></xsl:attribute>
  </xsl:template>
  <xsl:template mode="pattern-matching" match="sudoku:square" priority="1">
    <xsl:variable name="thisRow" select="@row"/>
    <xsl:variable name="thisCol" select="@col"/>
    <xsl:variable name="thisRegion" select="@region"/>
    <forbidden>
  ...
  <xsl:template mode="pattern-matching" match="sudoku:symbol" priority="1">
    ...
    <xsl:if test="
      ...
      | key('region-square',$thisRegion)[not(sudoku:allowed=$thisMarks) and contains(sudoku:allowed,$thisSymbol) and count(sudoku:allowed/sudoku:symbol)=count(  key('region-square-with-allowed-values',concat($thisRegion,'/',sudoku:allowed) )  ) ]
      "
    >
      <xsl:copy-of select="."/>
    </xsl:if>
